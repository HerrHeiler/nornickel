...
from src.config import InferenceConfig, ORE_TALC, ORE_REFRACTORY
...
@st.cache_resource
def get_pipeline(tile: int, overlap: int, downscale: float,
                 max_mpx: float) -> OrePipeline:
    cfg = InferenceConfig(tile_size=tile, overlap=overlap,
                          downscale_factor=downscale, max_mpx=max_mpx)
...
def correction_block(pipeline: OrePipeline, artifacts: dict, stem: str):
    ...
    st.download_button("⬇️ Индексная маска (PNG)", mask_png.tobytes(),
                       file_name=f"{stem}_mask.png", mime="image/png")

    fixed = st.file_uploader("Исправленная маска (PNG, значения 0–3)",
                             type=["png"], key="fixed_mask")
    if fixed is None:
        return
    ...
    if fixed_mask is None or fixed_mask.shape != artifacts["mask"].shape:
        st.error("Маска не читается или не совпадает по размеру "
    ...
    st.dataframe(res_fixed.metrics_df, use_container_width=True,
                 hide_index=True)
    if st.button("💾 Сохранить пару для дообучения"):
        ...
        cv2.imencode(".png", cv2.cvtColor(
            artifacts["image"], cv2.COLOR_RGB2BGR))[1].tofile(
            str(corr / "images" / f"{stem}.png"))
        cv2.imencode(".png", fixed_mask)[1].tofile(
            str(corr / "masks" / f"{stem}.png"))
        st.success("Сохранено в data/corrections — добавьте папку в "
...
def main():
    with st.sidebar:
        ...
        px_size = st.number_input("Размер пикселя, мкм (0 = неизвестен)",
                                  min_value=0.0, value=0.0, step=0.1)
        max_mpx = st.select_slider("Лимит, Мпикс (панорамы даунскейлятся)",
                                   [25, 50, 100], value=100)
        show_conf = st.checkbox("Карта уверенности модели", value=False)
        st.divider()
        st.caption("Зелёный — обычные срастания · Красный — тонкие срастания · "
                   "Синий — тальк")

    st.title("🔬 Автоматическая классификация руд по панорамным OM-снимкам шлифов")

    uploaded = st.file_uploader("Загрузите панорамное изображение шлифа",
    ...
    st.caption(f"Время обработки: {artifacts['elapsed_s']:.1f} с · "
    ...
    if show_conf:
        zoomable(confidence_heatmap(artifacts["confidence"]),
    ...
    mcol4.metric("Тальк", f"{res.talc_pct:.1f}%",
                 delta="порог 10%", delta_color="off")

    st.dataframe(res.metrics_df, use_container_width=True, hide_index=True)

    csv_buf = io.StringIO()
    res.metrics_df.to_csv(csv_buf, index=False)
    st.download_button("⬇️ Экспорт метрик (CSV)", csv_buf.getvalue(),
                       file_name=f"{stem}_metrics.csv", mime="text/csv")

    ok, jpg = cv2.imencode(".jpg",
                           cv2.cvtColor(artifacts["overlay"], cv2.COLOR_RGB2BGR),
                           [cv2.IMWRITE_JPEG_QUALITY, 92])
    st.download_button("⬇️ Экспорт маски (JPEG)", jpg.tobytes(),
                       file_name=f"{stem}_overlay.jpg", mime="image/jpeg")
